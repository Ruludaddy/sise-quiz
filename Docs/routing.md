...
Granite 앱에서는 화면 전환, 히스토리 관리, 파라미터 전달 같은 라우팅을 간편하게 처리할 수 있어요. 내부적으로는 [React Navigation](https://reactnavigation.org/)을 기반으로 동작하므로, 익숙한 API로 바로 활용할 수 있습니다.
...
![navigate-state-1](/assets/navigate-flow.Qz_u4iRx.webp)

페이지 A → 페이지 B → 페이지 C 순서로 이동한 직후의 상태는 아래와 그림과 같이 같이 나타낼 수 있어요.

![navigate-state-1](/assets/navigate-state-1.B0Q9MCl_.webp)
...
![navigate-state-2](/assets/navigate-state-2.BXzLEA1S.webp)
